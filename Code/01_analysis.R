...
# This is the final date that is used in the paper
...
data <- readRDS(paste0("/home/skerr/Data/ccp_subset_derived_", time_stamp, ".rds")) %>%
  ...
  mutate(
    sf94_dd = case_when(
      days_since_start >= day_of_death ~ 0.5,
      days_since_start >= day_of_discharge ~ 4.76,
      TRUE ~ sf94
    ),
    who_dd = case_when(
      days_since_start >= day_of_death ~ 10,
...
dead_alive_to_add <- function(df, days) {
  ...
  for (day in days) {
    ...
    row <- c(
      "% dead" = dead, "% alive" = alive, "% in study" = percentage_in_study,
...
calculate_cstat_by_day <- function(df, days) {
  ...
  c_stat_out <- mutate(c_stat_out,
    C_statistic = as.numeric(C_statistic),
    day = as.numeric(day)
  ) %>%
    arrange(model, day)
...
write.csv(
  dead_alive_summary_cum(df_sf94 %>% filter(!is.na(sf94_day5_P)), c(5, 8, 10:16)),
...
write.csv(
  dead_alive_summary_cum(df_sf94 %>% filter(!is.na(sf94_day8_P)), c(5, 8, 10:16)),
...
calculate_sample_size_mort <- function(mort, mort_multiplier) {
  ...
  out <- c(
    "p1" = p1, "p2" = p2, "mortality_multiplier" = mort_multiplier,
...
calculate_sample_size_lr <- function(df, endpoint_var, mort_multiplier, alpha, power, a, f) {
  ...
  out <- c(
    "p1" = p1, "p2" = p2, "mortality_multiplier" = mort_multiplier, "hazard_ratio" = HR,
...
df_sf94_sample_size <- rbind(
  ...
  c(
    calculate_sample_size_sf94(
      0.05, 0.8, df_sf94_effect_size["subset2", "day5_P"],
      df_sf94_sd["subset2", "day5_P"],
      df_sf94_corr["subset2", "day5_P"]
    ),
    calculate_sample_size_sf94(
      0.05, 0.8, df_sf94_effect_size["subset2", "day8_P"],
  ...
  c(
    ...
    calculate_sample_size_sf94(
      0.05, 0.8, df_sf94_effect_size["subset3", "day8_P"],
...
df_who_sample_size <- rbind(
  ...
  # subset2
  c(
    calculate_sample_size_who(0.05, 0.8, df_who_effect_size["subset2", "day5_P"], who_prop_table["subset2_day5_P", ]),
    calculate_sample_size_who(0.05, 0.8, df_who_effect_size["subset2", "day8_P"], who_prop_table["subset2_day8_P", ])
  ),

  # subset3
  c(
    calculate_sample_size_who(0.05, 0.8, df_who_effect_size["subset3", "day5_P"], who_prop_table["subset3_day5_P", ]),
...
calculate_sample_size_susimp <- function(power, effect_size, p1) {
  ...
  # Calculate sample size for a proportions test
...
df_susimp_sample_size <- rbind(
  ...
  c(
    calculate_sample_size_susimp(
      0.8, df_susimp_effect_size["subset2", "sustained_1L_improvement"],
      susimp_prop_table["subset2", "sustained_1L_improvement"]
    ),
    calculate_sample_size_susimp(
      0.8, df_susimp_effect_size["subset2", "sustained_2L_improvement"],
  ...
  c(
    calculate_sample_size_susimp(
      0.8, df_susimp_effect_size["subset3", "sustained_1L_improvement"],
      susimp_prop_table["subset3", "sustained_1L_improvement"]
    ),
    calculate_sample_size_susimp(
      0.8, df_susimp_effect_size["subset3", "sustained_2L_improvement"],
...
sf94_prot_effect_size <-
  c(
    effect_size_boot_sf94_prot(subset1, 1:nrow(subset1), treatment = 0.85, sd_mult = 1, rho_opp_prot = 0.5),
    effect_size_boot_sf94_prot(subset1, 1:nrow(subset1), treatment = 0.85, sd_mult = 1, rho_opp_prot = 0.6),
    effect_size_boot_sf94_prot(subset1, 1:nrow(subset1), treatment = 0.85, sd_mult = 1, rho_opp_prot = 0.7),
    effect_size_boot_sf94_prot(subset1, 1:nrow(subset1), treatment = 0.85, sd_mult = 1, rho_opp_prot = 0.8),
    effect_size_boot_sf94_prot(subset1, 1:nrow(subset1), treatment = 0.85, sd_mult = 1, rho_opp_prot = 0.9)
  )
