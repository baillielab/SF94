...
# Creates a dataframe consisting of variable measurements up until time t.
# People who died or were discharged are given appropriate values on day of discharge/death
...
createDF <- function(df, group, variable, time){
  ...
  derived <-  df2  %>%  dplyr::mutate(rn= row_number()) %>% spread (days_since_start, variable) %>% dplyr::select(-rn)
...
# Steven, 20/06/2022: This causes people to be missed out in table_function counts
# The problem is that e.g. if they have a non-NA value for sf94 on day 0 and they die, then
# they will not be counted amongst the people with sf94_day0 == 0.5, due to the way the 
# case_when statement works!
