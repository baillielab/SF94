...
ggplot() +
  geom_col(data = subset(df_dd, group_dd == "Alive"), 
           aes(x = count_dd, y = sf94_dd, fill = 'Alive')) +
  geom_col(data = subset(df_dd, group_dd == "Dead"), 
           aes(x = count_dd, y = sf94_dd, fill = 'Dead'))+ coord_flip()
...
ggplot(df_hist, aes(x = sf94, fill = group)) + 
  ...
  theme_light() +
  xlab("S/F94 day 5") +
  scale_fill_manual(values=c("purple","black"),breaks=c("original", "dd"),
