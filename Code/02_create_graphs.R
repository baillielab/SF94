...
sample_size_mort <- calculate_sample_size_mort_vec(mean_death, c(0.85, 0.8, 0.75, 0.7)) %>%
  ...
  data.frame() %>%
  mutate(
    outcome_measure = "28-day mortality",
...
effect_size_sf94 <- data.frame(
  outcome_measure = rep("S/F94 day 5", 4),
  treatment_effect = c(0.85, 0.8, 0.75, 0.7),
...
effect_size_who <- data.frame(
  outcome_measure = rep("WHO day 5", 4),
  treatment_effect = c(0.85, 0.8, 0.75, 0.7),
...
who_sample_size <- c(
  calculate_sample_size_who(0.05, 0.8, who_effect_size[1], who_prop_table["subset1_day5_P", ]),
...
who_sample_size_ucl <- c(
  calculate_sample_size_who(0.05, 0.8, who_effect_size_ucl[1], who_prop_table["subset1_day5_P", ]),
...
who_sample_size_lcl <- c(
  calculate_sample_size_who(0.05, 0.8, who_effect_size_lcl[1], who_prop_table["subset1_day5_P", ]),
...
effect_size_susimp <- data.frame(
  outcome_measure = rep("Sustained 1 level improvement", 4),
  treatment_effect = c(0.85, 0.8, 0.75, 0.7),
...
effect_size_susimp2 <- data.frame(
  outcome_measure = rep("Sustained 2 level improvement", 4),
  treatment_effect = c(0.85, 0.8, 0.75, 0.7),
...
sample_size_susimp2 <- data.frame(
  outcome_measure = rep("Sustained 2 level improvement", 4),
  treatment_effect = c(0.85, 0.8, 0.75, 0.7),
...
# sd_mult doesn't actually get used now because we have data from recovery trial
# sd_mult = 0.8
sd_mult <- 1.25 / sd_opp
...
effect_size_sf94_prot <- data.frame(
  outcome_measure = rep("Protocolised S/F94 day 5", 4),
  treatment_effect = c(0.85, 0.8, 0.75, 0.7),
...
sample_size_sf94_prot <- data.frame(
  outcome_measure = rep("Protocolised S/F94 day 5", 4),
  treatment_effect = c(0.85, 0.8, 0.75, 0.7),
...
df_effect_size <- rbind(
  effect_size_sf94, effect_size_who, effect_size_susimp, effect_size_susimp2,
...
df_sample_size <- rbind(
  sample_size_mort, sample_size_sf94, sample_size_who, sample_size_susimp,
...
df_sample_size$outcome_measure <- gsub(
  "Sustained 1 level improvement", "WHO sustained 1-level improvement",
  df_sample_size$outcome_measure
)

df_sample_size$outcome_measure <- gsub(
  "Sustained 2 level improvement", "WHO sustained 2-level improvement",
  df_sample_size$outcome_measure
)
...
ggplot(
  ...
  geom_ribbon(aes(ymin = sample_size_lcl, ymax = sample_size_ucl),
    linetype = 1, alpha = 0.2, colour = NA,
    show.legend = F
  ) +
...
df_countplot
head(df_countplot)
days_since_start <- rep(c(1:12), 2)
count_dd <- sample.int(100, 24, replace = T) # number of extreme values added per day
group_dd <- c(rep("Alive", 12), rep("Dead", 12))
sf94_dd <- c(rep(4.76, 12), rep(0.5, 12))
df_dd <- data.frame(days_since_start, count_dd, group_dd, sf94_dd) # combine to dataframe with days 1-12
head(df_dd)

ggplot() +
  geom_col(
    data = subset(df_dd, group_dd == "Alive"),
    aes(x = count_dd, y = sf94_dd, fill = "Alive")
  ) +
  geom_col(
    data = subset(df_dd, group_dd == "Dead"),
    aes(x = count_dd, y = sf94_dd, fill = "Dead")
  ) +
  coord_flip()
...
df <- dplyr::select(data, subjid, days_since_start, sf94, mortality_28) %>%
  ...
  mutate(
    days_since_start = factor(as.character((days_since_start)), levels = 0:12),
...
data_1 = data_1 %>%
  mutate(
    IMV = case_when(severity_scale_ordinal %in% c(9, 8, 7) ~ 'Yes',
                    TRUE ~ 'No')
  )
...
df <- dplyr::select(data, subjid, days_since_start, sf94, mortality_28) %>%
  ...
  mutate(
    days_since_start = factor(as.character((days_since_start)), levels = 0:28),
    mortality_28 = as.factor(mortality_28)
  )
...
ggsave(
  plot = plot_d5_multi, dpi = 300, path = paste0("/home/skerr/Git/SF94/Outputs/", time_stamp), filename = "day_5_predicted_mortality_multivariate_model_plot.pdf",
...
ggplot(df_hist, aes(x = sf94, fill = group)) +
  ...
  xlab("S/F94 day 5") +
  scale_fill_manual(
    values = c("purple", "black"), breaks = c("original", "dd"),
